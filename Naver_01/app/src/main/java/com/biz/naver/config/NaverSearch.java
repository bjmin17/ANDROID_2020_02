...
public class NaverSearch extends AsyncTask<Integer,Integer,Void> {
    ...
    /*
                ...
                매개변수가 몇개인지 관계없이 어떤 부분이라도 이 메서드를 호출할 수 있다.
                매개변수의 개수가 정해지지 않은 호출방식
                doInBackground(3,4,5,6,7,8,9,0)는
                Integer[] integers = new Integer[8]로 선언되는것과 같다.
    ...
    // 비동기라서 언제 끝날지 모름
    // 끝났을 때 알려줄 메서드(onPostExecute)를 위에 하나 더 생성
    public void naver_search() {
        ...
        try {
            ...
            apiURL += "&start=1";
            apiURL += "&display=20"; // 한번에 20개씩 보여달라
