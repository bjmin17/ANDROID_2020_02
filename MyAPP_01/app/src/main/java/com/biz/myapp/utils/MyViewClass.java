...
/*
    ...
    어떤 이벤트나 액션 등을 지정할 때
    기존의 클래스나 인터페이스를 상속 또는 implement 해야하는데
    그때, 자기 자신의 클래스를 사용하지 않고 View 클래스의 요소들을 상속받아서
    클래스를 작성하는 것이 원칙이다.
    아래 implements View.~~ 부분에 대한 설명.
 */
public class MyViewClass implements View.OnClickListener {
    ...
    public void onClick(View v) {
        ...
        누가, 호출했는가를 알고 싶을때
        v.getId() method를 사용하면 호출한 view(comp)의 id값을 얻어올 수 있다.
        ...
        if(v.getId() == R.id.btn1){
            ...
            /*
            이벤트가 발생한 view(comp)로부터 어떤 값을 얻어오고자 하면
